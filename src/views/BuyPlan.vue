<script setup lang="ts">
  ...
  const fetchPlans = async (): Promise<void> => {
    ...
    try {
      ...
      message.error(error.message) // Hiển thị thẳng message từ service
    } finally {
      loading.value = false
    }
  }
  ...
  <div class="container buy-plan-page">
    <div class="card p-3 mb-4">
      ...
      <div class="d-flex align-items-center flex-column flex-md-row">
        <img
          ...
          class="thumbnail mb-3 mb-md-0"
          alt="Thumbnail"
        />
        <div class="ms-md-3 text-center text-md-start">
          <h6>{{ post.title }}</h6>
          <p class="text-muted mb-0">
            {{ Number(post.price).toLocaleString() }} VNĐ - {{ post.area }}m² - {{ post.regionAddress }}
          </p>
        </div>
      </div>
    </div>
      ...
      <div v-else class="row">
        <div class="col-12 col-md-4 mb-3" v-for="plan in plans" :key="plan.id">
          <div
            ...
            :style="{ borderColor: getBorderColor(plan.priorityLevel) }"
          >
    ...
    >
      <div v-if="selectedPlan">
        <p>Bạn có chắc chắn muốn mua gói này cho bài đăng "<strong>{{ post.title }}</strong>"?</p>
        <p>Độ ưu tiên: <strong>{{ selectedPlan.priorityLevel }}</strong></p>
        <p>Thời gian: <strong>{{ selectedPlan.duration }}</strong> ngày</p>
        <p>Giá: <strong>{{ selectedPlan.price.toLocaleString() }}</strong> VNĐ</p>
      </div>
    </a-modal>
  </div>
</template>
